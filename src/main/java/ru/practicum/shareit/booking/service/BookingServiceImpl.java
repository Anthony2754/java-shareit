...
public class BookingServiceImpl implements BookingService {
    ...
    @Override
    public boolean commentMadeAfterBooking(long bookerId, long itemId) {
    ...
    private boolean freeBookingTime(Booking booking) {
        ...
        if (activeBookings.size() > 0) {
            for (Booking activeBooking : activeBookings) {
                dataTimeStart = activeBooking.getStartTime();

                if (dataTimeNow.isBefore(startTime) && dataTimeStart.isAfter(endTime)) {
                    return true;

                } else if (dataTimeNow.isAfter(endTime)) {
                    return false;
                }
                dataTimeNow = dataTimeStart;
            }
            return dataTimeNow.isBefore(startTime);
